...
const AboutUs: FC = () => {
  ...
  const makeOpenAiRequest = async () => {
    ...
    try {
      ...
      let gptPrompt = openAIPrompt
        .replace(`[topic]`, articleTopic)
        .replace(`[keywords]`, keywordsList)
        .replace(`[length]`, articleLength);
      ...
      <Box>
        <Stack spacing={5} mt={10}>
          <Typography mt={2} variant="h5">
            Put here the news & the article from the website
          </Typography>
          <TextField
        ...
        <Box mt={5}>
          <Button
            disabled={gptResponseLoading}
            size="large"
            variant="contained"
            onClick={makeOpenAiRequest}
          >
            Write Article
          </Button>
        </Box>
